...
export const A = {
  ...
  spendingStrategy:
    "How much you spend each retirement year — distinct from the Withdrawal Policy, which only picks which account to tap. Steady Paycheck funds your plan's expense streams every year, optionally flexed by Guyton-Klinger guardrails that trim or restore discretionary spending as your withdrawal rate runs hot or cool. Percent of Portfolio instead sets discretionary spending from a percentage of your accessible (penalty-free) wealth, so it self-corrects with the market and never depletes to zero — at the cost of a variable income; toggles let the rate rise with age (VPW), bound it to a floor and ceiling of plan, or smooth it year to year. In both, essentials (medical + loan payments) are funded first, and a path still fails if accessible wealth can't cover them.",
  taxRegime:
  ...
  bridgeCoverage:
    "Penalty-free assets the moment you retire ÷ the total spending the bridge to 59½ demands, ignoring market growth — a floor, not a forecast. 1× means you could just reach 60 with markets flat; below 1× in the worst 5% means bad sequences force you onto growth or penalized withdrawals. A day-one snapshot that doesn't credit conversions maturing mid-bridge, so for a ladder-reliant plan it understates the real cushion — trust Bridge Holds for the verdict.",
  bridgePenalty:
